...
/**
 * Escape a message string.
 *
 * @param {string} message - The message to escape.
 * @param {(error: string) => unknown} [onerror] - A callback to run on invalid characters.
 *
 * @returns {string} - The escaped message.
 */
function escapeMessage(message, onerror) {
	return message
		.replace(/&/g, "&amp;")
		.replace(/</g, "&lt;")
		.replace(/>/g, "&gt;")
		.replace(/"/g, () => {
			if (onerror) onerror("non-curly double quotes");

			return "&quot;";
		})
		.replace(/['`‘]/g, () => {
			if (onerror) onerror("non-curly single quotes");

			return "&#8217;";
		})
		.replace(/\.{3}/g, () => {
			if (onerror) onerror("triple dots");
...
function parseTranslations(messages) {
	...
	for (const key in messages) {
		...
		if (message?.string) {
			returnValue[`${key}`] = escapeMessage(message.string, (error) =>
				logError(
					`The \`${key}\` string (translated as \`${message.string}\`) contains ${error}!`,
				),
			);
		...
		for (const subKey in message) {
			...
			if (typeof message[`${subKey}`]?.string === "string") {
				returnValue[`${key}.${subKey}`] = escapeMessage(
	...
	const returnValue = languages
		...
		.flatMap((language) => {
			...
			const [noCountryLanguage] = language.split("_"),
				...
				[noDialectLanguage] = language.split("@");
