...
function parseTranslations(messages) {
	...
	for (const key in messages) {
		...
		if (typeof message?.string !== "undefined") {
		...
		for (const subKey in message) {
			...
			// @ts-expect-error -- It's impossible for `message[`${subKey}`]` to be the wrong type.
...
function splitOnNotBetween(toSplit, splitOn, notStart, notEnd) {
	...
	return Array.from(toSplit)
			...
			(
				...
				if (currentValue[index - 1] === "\\") {
					...
					if (value !== splitOn || unbalancedParens) {
						...
						accumulated.concat(currentValue);
						currentValue = "";
		...
		.concat(currentValue)
		.map((toUnescape) => toUnescape.replace(/\\;/g, ";"));
}
